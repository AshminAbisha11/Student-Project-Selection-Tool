...
describe('cycleController.getStatus', () => {
  it('returns hasActiveCycle=false when no records', async () => {
    const req = mockReq();
    const res = mockRes();
    ...
    expect(res.json).toHaveBeenCalledWith({ hasActiveCycle: false });
  });

  it('returns status for the most relevant (open) cycle', async () => {
    const now = Date.now();
    const oneHour = 60 * 60 * 1000;
    const closeAt = new Date(now + oneHour).toISOString();
    const commitAt = new Date(now + 2 * oneHour).toISOString();
    ...
    db.query.mockResolvedValueOnce([[
      {
        ...
        commit_at: commitAt,
        submission_open_at: new Date(now - oneHour).toISOString(),
        name: 'Cycle X'
      }
    ]]);
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        cycle: expect.objectContaining({ cycle_id: 10, status: 'open' })
      })
    );
  });
});
...
describe('cycleController.create', () => {
  ...
  it('201 creates draft (no status open logic), returns inserted row', async () => {
    const req = mockReq({
      body: {
        name: 'C1',
        submission_open_at: '2025-01-02 09:00:00',
    ...
    // INSERT
    db.query
      .mockResolvedValueOnce([{ insertId: 77 }]) // insert cycle
      .mockResolvedValueOnce([[{ cycle_id: 77, name: 'C1', status: 'draft' }]]); // select row
...
describe('cycleController.update', () => {
  ...
  it('400 when no fields to update', async () => {
    ...
    db.query.mockResolvedValueOnce([[{ cycle_id: 5, submission_open_at: '2025-01-02 09:00:00', submission_close_at: '2025-01-03 09:00:00', commit_at: null }]]);

    await cycleCtl.update(req, res);
    expect(res.status).toHaveBeenCalledWith(400);
  });

  it("status='open' closes other and updates row", async () => {
    const req = mockReq({
      params: { id: 9 },
...
describe('cycleController.openNow', () => {
  it('opens cycle and seeds (we just assert success message)', async () => {
    const req = mockReq({ params: { id: 42 } });
    const res = mockRes();

    // Build a connection mock shape
    const conn = {
      ...
      query: jest.fn()
        ...
        // open this one
        .mockResolvedValueOnce([{ affectedRows: 1 }])
        // prev cycle query -> found a prev
        .mockResolvedValueOnce([[{ cycle_id: 41 }]])
        // 3a insert projects from prev
        .mockResolvedValueOnce([{ affectedRows: 3 }])
        // 3b copy details from prev
        .mockResolvedValueOnce([{ affectedRows: 3 }])
        // 4a insert from drafts
...
describe('cycleController.commitNow', () => {
  it('sets commit time to now', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ message: 'Commit time set to now' });
  });
});
